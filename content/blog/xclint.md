...
date = 2017-11-02T12:00:00+00:00
slug = "xclint"
...
* There are entries that are duplicated *(a common issue when solving git conflicts in the project file)*. - There are elements that refer to others that don't exist anymore. - There are attributes that are missing. - Some file references point to files that don't exist anymore.
...
![xclint output when there are warnings](__GHOST_URL__/images/posts/xclint.png)
...
* Detect multiple elements with the same reference. - Spot files that are referred from the project that are missing.
