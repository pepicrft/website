...
date = 2015-06-18T12:00:00+00:00
slug = "paginated-api-requests-using-functional-reactive-in-swift"
...
* **Subscribe next**: The approach uses recursive subscribing to the next signal and passing the subscriber through. Subscribe next closure takes the source subscriber and depending on the next signal results it: - *Closes* the stream: Sending a completion or a failure message - *Sends* the results through the stream - Subscribes to the *next signal* when the results count is equal to the page limit
